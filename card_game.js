...
function shuffle(deck) {
  ...
  randomValue = parseInt((Math.random() * 52))
  temporaryValue= shuffledDeck[currentIndex] 
  shuffledDeck[currentIndex] = shuffledDeck[randomValue]
  shuffledDeck[randomValue] = temporaryValue
  currentIndex -= 1; 
  }
  return shuffledDeck
}
...
function greet() {
  ...
  console.log("welcome to this game!" + name)  
  return name
  }
  // greet()
...
function guess(card1, card2){
  ...
  if (input === 'h'){
    ...
    console.log("Guess (h) or (l) only and you will not get points for this round")
    return false
  }
...
function playGame(){
  ...
  let currentCard = deck.pop()
    ...
    if(guess(currentCard, nextCard)){
      ...
      console.log("You are wrong and will recieve no points... Im sorry")
    }
  currentCard = nextCard 
    }
 deck.length === 0 ? console.log("You have lost and are out of cards") : console.log("You won!")
